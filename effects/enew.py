...
class Dependent(Generic[KT, CT]):
    ...
    def trigger_update(self, key: KT, now: pd.Timestamp) -> None:
        self._dependent.trigger_update(self._convert(key), now)
...
class EffectBase(Generic[KT]):
    ...
    def on_update(self, key: KT, now: pd.Timestamp) -> None:
        for dependent in self._dependents:
            dependent.trigger_update(key, now)
...
class EffectDependent(Generic[KT, VT], EffectBase[KT]):
        ...
        try:
            while True:
                if self._thread is not threading.current_thread():
                    break
                next_tasks = self.pending_outdated()
                if next_tasks is None:
                    break
                for task in sorted(next_tasks, key=lambda task: task[1]):
                    key, when = task
                    now = now_ts()
    ...
    def pending_outdated(self) -> list[tuple[KT, pd.Timestamp]] | None:
        raise NotImplementedError()
    ...
    def do_set_value(self, key: KT, value: VT, now: pd.Timestamp) -> None:
        raise NotImplementedError()
    ...
    def do_update_value(
            self, key: KT, value: VT, now: pd.Timestamp) -> VT | None:
        raise NotImplementedError()
    ...
    def do_set_new_value(self, key: KT, value: VT, now: pd.Timestamp) -> bool:
        raise NotImplementedError()
