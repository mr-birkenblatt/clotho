...
class FusionInfo:
    ...
    def __init__(self, pattern, replacement_type_this_element, is_first_element, is_last_element) -> None: ...
...
class SeenQOpInfo:
    ...
    def __init__(self, idx, type, type_is_module, fqn, input_tensor_infos, output_tensor_infos, packable_tensor_idx_to_name, packable_nontensor_idx_to_arg, packable_tensor_kwarg_name_to_name, op_packing_only_uses_module_attributes, qconfig, fusion_info, is_reference_op_at_inference) -> None: ...

class SeenNonQOpInfo:
    type: Callable
    input_tensor_infos: List[Optional[QTensorInfo]]
    output_tensor_infos: List[QTensorInfo]
    def __init__(self, type, input_tensor_infos, output_tensor_infos) -> None: ...
...
def trace_with_inputs(model: torch.nn.Module, example_args: Tuple[Any]) -> None: ...
def is_leaf(m: torch.nn.Module, prepare_custom_config_dict: Optional[Dict[str, Any]]) -> bool: ...
...
def get_func_output_obs_type(seen_q_op_info: SeenQOpInfo) -> FuncOutputObsType: ...
...
def get_func_output_dtype_type(seen_q_op_info: SeenQOpInfo) -> FuncOutputDTypeType: ...
def get_weight_argument_info(op: Callable) -> Optional[Tuple[int, str]]: ...
def get_op_packing_only_uses_module_attributes(op: Callable, args: Tuple[Any, ...], kwargs: Dict[str, Any], module: torch.nn.Module) -> bool: ...
def get_quantized_op(seen_q_op_info: SeenQOpInfo, idx_to_seen_q_op_infos: Dict[int, SeenQOpInfo]) -> Optional[Callable]: ...
def get_input_observed_arg_idxs(op_type: Callable, op_type_is_module: bool) -> Optional[List[int]]: ...
...
def iterate_and_apply(args: Any, flattened_tensor_infos: List[Optional[QTensorInfo]], func: Callable, flattened_tensor_infos_idx: Incomplete | None = ...) -> Any: ...
def get_producer_of_seen_q_op_info(idx_to_seen_q_op_info: Dict[int, SeenQOpInfo], cur_seen_q_op_info: SeenQOpInfo) -> Optional[SeenQOpInfo]: ...
def get_users_of_seen_q_op_info(idx_to_seen_q_op_info: Dict[int, SeenQOpInfo], cur_seen_q_op_info: SeenQOpInfo) -> List[SeenQOpInfo]: ...
...
def get_torch_function_hook_type(parent_module: Optional[torch.nn.Module], func: Callable) -> HookType: ...
def get_module_hook_type(parent_module: Optional[torch.nn.Module], cur_module: torch.nn.Module) -> HookType: ...
def clone_detach_tensor_without_dispatch(x: torch.Tensor) -> torch.Tensor: ...
def get_input_args_quant_dequant_info(seen_q_op_info: SeenQOpInfo, tensor_id_to_scale_zp: Dict[int, Tuple[torch.Tensor, torch.Tensor]]) -> Tuple[List[Optional[Tuple[float, int, torch.dtype]]], List[bool], bool]: ...
def get_cur_qconfig(qconfig_dict: Dict[str, Any], cur_fqn: str, cur_op_type: Callable) -> Optional[QConfigAny]: ...
