import { assertEqual, assertNotEqual, assertTrue, range, SafeMap, SafeSet, union } from "./util";
...
test('SafeSet tests', () => {
  ...
  let sCount = 0;
  set.forEach((value) => {expect(ref.has(value.b)).toBe(true); sCount += 1});
  ...
  const map = new SafeMap(Array.from(other.values()).map(el => [el.a, el.b]));
  ...
  expect(new Set(Array.from(map.keys()))).toEqual(new Set([undefined, 'a', 'c', 'd']));
  ...
  const mSet = new SafeSet(Array.from(map.entries()).map(el => ({a: el[0], b: el[1]})));
  expect(new Set(Array.from(mSet.values()))).toEqual(new Set(Array.from(other.values())));
  map.clear();
...
test('union tests', () => {
  const origA = new Map([[0, 0], [1, 2]]);
  const origB = new Map([[2, 2], [3, 4]]);
  const origC = new Map([[1, 3], [2, 4]]);
  const resAB = new Map([[0, 0], [1, 2], [2, 2], [3, 4]]);
  const resAC = new Map([[0, 0], [1, 3], [2, 4]]);
  const resCA = new Map([[0, 0], [1, 2], [2, 4]]);
  const resBC = new Map([[1, 3], [2, 4], [3, 4]]);
  const resCB = new Map([[1, 3], [2, 2], [3, 4]]);
  const resABC = new Map([[0, 0], [1, 3], [2, 4], [3, 4]]);
