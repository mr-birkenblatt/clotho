# Stubs for pandas.core.dtypes.common (Python 3)
#
# NOTE: This dynamically typed stub was automatically generated by stubgen.
# pylint: disable=unused-argument,super-init-not-called,redefined-outer-name
# pylint: disable=arguments-differ,too-many-arguments
# pylint: disable=dangerous-default-value,too-many-ancestors
# pylint: disable=useless-import-alias,unused-import

from typing import Any, Union

import numpy as np
from pandas._typing import ArrayLike
from pandas.core.dtypes.inference import (  # noqa:F401
    is_array_like as is_array_like,
)
from pandas.core.dtypes.inference import is_bool as is_bool
from pandas.core.dtypes.inference import is_complex as is_complex
from pandas.core.dtypes.inference import is_decimal as is_decimal
from pandas.core.dtypes.inference import is_dict_like as is_dict_like
from pandas.core.dtypes.inference import is_file_like as is_file_like
from pandas.core.dtypes.inference import is_float as is_float
from pandas.core.dtypes.inference import is_hashable as is_hashable
from pandas.core.dtypes.inference import is_integer as is_integer
from pandas.core.dtypes.inference import is_interval as is_interval
from pandas.core.dtypes.inference import is_iterator as is_iterator
from pandas.core.dtypes.inference import is_list_like as is_list_like
from pandas.core.dtypes.inference import is_named_tuple as is_named_tuple
from pandas.core.dtypes.inference import (
    is_nested_list_like as is_nested_list_like,
)
from pandas.core.dtypes.inference import is_number as is_number
from pandas.core.dtypes.inference import is_re as is_re
from pandas.core.dtypes.inference import is_re_compilable as is_re_compilable
from pandas.core.dtypes.inference import is_scalar as is_scalar
from pandas.core.dtypes.inference import is_sequence as is_sequence
from pandas.core.dtypes.inference import is_string_like as is_string_like

ensure_float64: Any
ensure_float32: Any
ensure_uint64: Any
ensure_int64: Any
ensure_int32: Any
ensure_int16: Any
ensure_int8: Any
ensure_platform_int: Any
ensure_object: Any
_NS_DTYPE = Any
_TD_DTYPE = Any
_INT64_DTYPE = Any


def ensure_float(arr: Any) -> Any:
    ...


def ensure_str(value: Union[bytes, Any]) -> str:
    ...


def ensure_categorical(arr: Any) -> Any:
    ...


def ensure_int_or_float(arr: ArrayLike, copy: Any = ...) -> np.ndarray:
    ...


def ensure_python_int(value: Union[int, np.integer]) -> int:
    ...


def classes(*klasses: Any) -> Any:
    ...


def classes_and_not_datetimelike(*klasses: Any) -> Any:
    ...


def is_object_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_sparse(arr: Any) -> Any:
    ...


def is_scipy_sparse(arr: Any) -> Any:
    ...


def is_categorical(arr: Any) -> Any:
    ...


def is_datetimetz(arr: Any) -> Any:
    ...


def is_offsetlike(arr_or_obj: Any) -> Any:
    ...


def is_period(arr: Any) -> Any:
    ...


def is_datetime64_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_datetime64tz_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_timedelta64_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_period_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_interval_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_categorical_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_string_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_period_arraylike(arr: Any) -> Any:
    ...


def is_datetime_arraylike(arr: Any) -> Any:
    ...


def is_datetimelike(arr: Any) -> Any:
    ...


def is_dtype_equal(source: Any, target: Any) -> Any:
    ...


def is_any_int_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_integer_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_signed_integer_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_unsigned_integer_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_int64_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_datetime64_any_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_datetime64_ns_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_timedelta64_ns_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_datetime_or_timedelta_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_numeric_v_string_like(a: Any, b: Any) -> Any:
    ...


def is_datetimelike_v_numeric(a: Any, b: Any) -> bool:
    ...


def needs_i8_conversion(arr_or_dtype: Any) -> Any:
    ...


def is_numeric_dtype(arr_or_dtype: Any) -> bool:
    ...


def is_string_like_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_float_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_bool_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_extension_type(arr: Any) -> Any:
    ...


def is_extension_array_dtype(arr_or_dtype: Any) -> Any:
    ...


def is_complex_dtype(arr_or_dtype: Any) -> Any:
    ...


def infer_dtype_from_object(dtype: Any) -> Any:
    ...


def pandas_dtype(dtype: Any) -> Any:
    ...
