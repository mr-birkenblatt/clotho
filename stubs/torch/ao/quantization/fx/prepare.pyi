...
from .pattern_utils import MatchResult as MatchResult
        ...
        sorted_patterns_dict as sorted_patterns_dict
...
from .utils import BIAS_INDEX_DICT as BIAS_INDEX_DICT
        ...
        NON_QUANTIZABLE_WEIGHT_OPS as NON_QUANTIZABLE_WEIGHT_OPS,
        WEIGHT_INDEX_DICT as WEIGHT_INDEX_DICT,
...
def is_input_arg_dtype_supported_by_backend(
    ...
    Optional[Union[torch.dtype, type]]]], dtype_config: Dict[str,
    torch.dtype]) -> bool: ...
...
def prepare_get_standalone_module_configs(
    ...
    parent_backend_config_dict: Optional[Dict[str, Any]]) -> Tuple[Dict[str,
        Any], Dict[str, Any], Dict[str, Any]]: ...


def qat_swap_modules(
    root: torch.nn.Module, module_to_qat_module: Dict[Callable,
    Callable]) -> None: ...
...
def get_target_activation_dtype_for_node(
    ...
    cache_for_no_tensor_check: Dict[Node, bool]) -> Dict[str,
        Optional[Union[torch.dtype, type]]]: ...
...
def get_arg_target_dtype_as_output(
    ...
    node_name_to_target_dtype: Dict[str, Dict[str,
    Optional[Union[torch.dtype, type]]]]) -> Optional[Union[torch.dtype,
        type]]: ...
...
def maybe_insert_input_equalization_observers_for_node(
    ...
    modules: Dict[str, torch.nn.Module], graph: Graph,
    node_name_to_target_dtype: Dict[str, Dict[str,
    Optional[Union[torch.dtype, type]]]], is_branch: bool) -> None: ...
...
def maybe_insert_output_observer_for_node(
    ...
    node_name_to_target_dtype: Dict[str, Dict[str,
    Optional[Union[torch.dtype, type]]]], matched_pattern: Any,
    qhandler: Optional[QuantizeHandler], is_qat: bool) -> Optional[Node]: ...


def maybe_insert_observers_before_graph_output(
    graph_output_node: Node, output_quantized_idxs: List[int],
    node_name_to_target_dtype: Dict[str, Dict[str,
    Optional[Union[torch.dtype, type]]]], qconfig_map: Dict[str, QConfigAny],
    model: torch.nn.Module, modules: Dict[str, torch.nn.Module],
    graph: Graph) -> None: ...
...
def insert_observers_for_model(
    ...
    Any]], observed_node_names: Set[str], is_qat: bool) -> Optional[Node]: ...


def run_prepare_fx_on_standalone_modules(
    model: torch.nn.Module, is_qat: bool, modules: Dict[str,
    torch.nn.Module], matches: Any, prepare_custom_config_dict: Dict[str,
    Any], backend_config_dict: Optional[Dict[str, Any]]) -> None: ...
...
def save_state(
    ...
    Dict[Any, Any]], is_qat: bool, observed_node_names: Set[str]) -> None: ...
